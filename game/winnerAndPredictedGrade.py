...
try:
    ...
    def predictAverage(userID, examID):
        dbQuery = "SELECT `guess_mark` FROM `Game_bet` WHERE `exam_ID` = \'" + \
            examID + "\' AND `user_id` = \'" + userID + "\'"
    ...
    def decideRewardWinner(targetMark, targetID, examID):
        ...
        dbQuery = "SELECT `bet_id`, `exam_id`, `user_id`, `target_id`, `guess_mark`, `win` FROM `Game_bet` WHERE `exam_ID` = \'" + \
            examID + "\' AND `target_id` = \'" + targetID + "\'"
        mycursor.execute(dbQuery)
        currentBets = mycursor.fetchall()
        for bet in currentBets:
        ...
        for bet in currentBets:
            ...
            if smallestDifference == abs(bet[4] - targetMark):
                ...
                dbQuery = "UPDATE `Game_bet` SET `win` = b'1' WHERE `bet_id` = \'" + \
                    bet[0] + "\'"
                mycursor.execute(dbQuery)
                mydb.commit()
                # reward with credits: groups table
                # get credits of the user
                dbQuery = "SELECT `credits` FROM `Game_groupmember` WHERE `group_id` = \'" + \
                    groupID[0] + "\' AND `user_id` = \'" + bet[2] + "\'"
                mycursor.execute(dbQuery)
                winnerHasCredits = mycursor.fetchone()
                # update credits of the user
                updateWithCredits = wonCredits + winnerHasCredits[0]
                dbQuery = "UPDATE `Game_groupmember` SET `credits` = \'" + \
                    str(updateWithCredits) + \
                    "\'WHERE `user_id` = \'" + bet[2] + "\'"
                mycursor.execute(dbQuery)
                mydb.commit()
            # set all loosers as loosers
            elif smallestDifference < abs(bet[4] - targetMark):
                dbQuery = "UPDATE `Game_bet` SET `win` = b'0' WHERE `bet_id` = \'" + \
                    bet[0] + "\'"
